...
CREATE TABLE `Stickers` (
    ...
    UNIQUE INDEX `Stickers_key_key`(`key`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Todos` (
    ...
    `status` ENUM('pending', 'success', 'retry', 'archive') NOT NULL DEFAULT 'pending',
    `priority` ENUM('MUST_TODAY', 'GOOD_TODAY', 'DONT_FORGET') NOT NULL DEFAULT 'GOOD_TODAY',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
...
CREATE TABLE `DailyReviews` (
    ...
    `memo` TEXT NOT NULL,
    `compareNote` VARCHAR(191) NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    UNIQUE INDEX `DailyReviews_userId_date_key`(`userId`, `date`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
